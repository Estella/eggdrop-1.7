 ...
 *
 * $Id: botnet.c,v 1.11 2004/11/26 05:35:27 wcc Exp $
 */
...
{
  tandbot = NULL;
  maxparty = 50; /* Expand later as needed. */
...
void unvia(int idx, tand_t *who)
{
  ...
  /* Every bot unvia's bots behind anyway, so why send msg's for EVERY
  one? - will this break things?! */
...
void answer_local_whom(int idx, int chan)
{
  ...
  for (i = 0; i < dcc_total; i++) {
    if (dcc[i].type != &DCC_CHAT || (chan != -1 && (chan < 0 ||
        dcc[i].u.chat->channel != chan)))
      continue;
    t = strlen(dcc[i].nick);
    if (t > nicklen)
      nicklen = t;
  ...
  for (i = 0; i < dcc_total; i++) {
    ...
    dprintf(idx, format, c, dcc[i].nick,
            ...
            (dcc[i].u.chat->channel >= GLOBAL_CHANS && chan == -1) ? '*' :
  ...
  for (i = 0; i < parties; i++) {
    ...
    dprintf(idx, format, c, party[i].nick, ((party[i].chan == 0) &&
            (chan == -1)) ? '+' : ' ', party[i].bot, party[i].from, idle);
...
/* Show idx a list of all bots connected. */
...
void tell_bottree(int idx, int showver)
{
  ...
  int lev = 0, more = 1, tothops = 0, mark[20], ok, cnt, i, imark;
  ...
  if (showver)
    dprintf(idx, "%s (%d.%d.%d.%d)\n", botnetnick,
            egg_numver / 1000000, egg_numver % 1000000 / 10000,
      ...
      while (!s[0]) {
        if (bot->uplink == this) {
          if (bot->ver) {
            ...
            if (showver)
              sprintf(s + i, " (%d.%d.%d.%d)",
                      bot->ver / 1000000, bot->ver % 1000000 / 10000,
  ...
  for (i = 0; i < dcc_total; i++) {
    ...
    if (dcc[z].u.bot->numver < NEAT_BOTNET) {
      ...
      l = simple_sprintf(x, "i %s %D %D %s\n", botnetnick, dcc[i].sock,
                         now - dcc[i].timeval, dcc[i].u.chat->away ?
                         dcc[i].u.chat->away : "");
    tputs(dcc[z].sock, x, l);
  }
...
#endif
        l = simple_sprintf(x, "i %s %D %D %s\n", party[i].bot, party[i].sock,
                           now - party[i].timer, party[i].away ?
                           party[i].away : "");
      tputs(dcc[z].sock, x, l);
    }
  }
...
{
  if (findbot(who) || !egg_strcasecmp(who, botnetnick))
    return 1;
  return 0;
}
...
int botunlink(int idx, char *nick, char *reason, char *from)
{
    ...
    if (dcc[i].type == &DCC_FORK_BOT) {
      if (idx >= 0)
        dprintf(idx, "%s: %s -> %s.\n", BOT_KILLLINKATTEMPT, dcc[i].nick,
                dcc[i].host);
      putlog(LOG_BOTS, "*", "%s: %s -> %s:%d", BOT_KILLLINKATTEMPT, dcc[i].nick,
      ...
      putlog(LOG_BOTS, "*", "%s %s @ %s:%d", "Stopped trying to link",
  ...
  if (nick[0] != '*') {
    bot = findbot(nick);
    if (bot) {
      /* The internal bot list is desynched from the dcc list sometimes. While
       * we still search for the bug, provide an easy way to clear out those
       * `ghost'-bots. Fabian (2000-08-02)  */
...
static void botlink_resolve_success(int i)
{
  ...
  nfree(linker);
  if (dcc[i].sock < 0 ||
      open_telnet_raw(dcc[i].sock, iptostr(htonl(dcc[i].addr)), dcc[i].port) < 0)
    failed_link(i);
}
...
static void failed_tandem_relay(int idx)
{
  ...
  dcc[idx].port++;
  dcc[idx].timeval = now;
  if (dcc[idx].sock < 0 || open_telnet_raw(dcc[idx].sock, dcc[idx].addr ?
      iptostr(htonl(dcc[idx].addr)) : dcc[idx].host, dcc[idx].port) < 0)
...
static void tandem_relay_resolve_failure(int idx)
{
  ...
  if (uidx < 0) {
    putlog(LOG_MISC, "*", "%s  %d -> %d", BOT_CANTFINDRELAYUSER, dcc[idx].sock,
           dcc[idx].u.relay->sock);
    killsock(dcc[idx].sock);
    lostdcc(idx);
    return;
  }
...
static void tandem_relay_resolve_success(int i)
{
  ...
  dcc[i].timeval = now;
  if (open_telnet_raw(dcc[i].sock, iptostr(htonl(dcc[i].addr)), dcc[i].port) < 0)
...
static void eof_dcc_relay(int idx)
{
  ...
  check_tcl_chjn(botnetnick, dcc[j].nick, dcc[j].u.chat->channel, geticon(j),
                 dcc[j].sock, dcc[j].host);
  killsock(dcc[idx].sock);
  lostdcc(idx);
}
...
void check_botnet_pings()
{
  ...
  for (i = 0; i < dcc_total; i++) {
    if (dcc[i].type == &DCC_BOT && (dcc[i].status & BSTAT_PINGED)) {
      char s[1024];
      ...
      simple_sprintf(s, "%s: %s (lost %d bot%s and %d user%s)", BOT_PINGTIMEOUT,
                     dcc[i].nick, bots, (bots != 1) ? "s" : "", users,
                     (users != 1) ? "s" : "");
      chatout("*** %s\n", s);
      botnet_send_unlinked(i, dcc[i].nick, s);
      killsock(dcc[i].sock);
...
void restart_chons()
{
  ...
  for (i = 0; i < dcc_total; i++) {
    ...
    check_tcl_chjn(botnetnick, dcc[i].nick, dcc[i].u.chat->channel, geticon(i),
                   dcc[i].sock, dcc[i].host);
  }
  for (i = 0; i < parties; i++)
    check_tcl_chjn(party[i].bot, party[i].nick, party[i].chan, party[i].flag,
                   party[i].sock, party[i].from);
}
